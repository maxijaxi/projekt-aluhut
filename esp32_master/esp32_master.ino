// =============================================================
// PROJEKT ALUHUT – ESP32 Master
// Hardware : ESP32 + BLE (Master, verbindet zu HM-10 Slave)
//            Joystick X=GPIO36  Y=GPIO39  SW=GPIO15
//            RFID RC522  SS=GPIO5  RST=GPIO4
//            OLED SH1106 128x64 via I2C (SDA=GPIO21 SCL=GPIO22)
//            SPI für MFRC522: SCK=18 MISO=19 MOSI=23 SS=5
//
...
// ----------------------------------------------------------
// BLE KONFIGURATION  (Standard HM-10 UUIDs)
// ----------------------------------------------------------
#define SERVICE_UUID        "0000FFE0-0000-1000-8000-00805F9B34FB"
#define CHARACTERISTIC_UUID "0000FFE1-0000-1000-8000-00805F9B34FB"
#define SLAVE_ADDRESS       "78:04:73:c5:07:81"   // MAC-Adresse des HM-10
...
// Analoge Schwellwerte für 12-bit ADC (0–4095)
...
// ----------------------------------------------------------
// BLE-Empfangspuffer  (wird im Notify-Callback gefüllt)
// ----------------------------------------------------------
char             bleRxBuf[MAX_PACKET_SIZE + 1];
volatile uint8_t bleRxLen = 0;
char             bleRxReady[MAX_PACKET_SIZE + 1]; // Kopie zur Verarbeitung in loop
...
// ==========================================================
// UTIL: XOR-VERSCHLÜSSELUNG  (symmetrisch)
//       Ausgabe als Hex-String → keine Steuerzeichen im Stream
// ==========================================================
...
// ==========================================================
// BLE: NOTIFY-CALLBACK
// Wird vom BLE-Stack aufgerufen wenn der Slave Daten schickt.
// ==========================================================
void notifyCallback(NimBLERemoteCharacteristic *c,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (ch == '\n') {
            ...
            if (bleRxLen > 0 && !bleNewMsg) {
                // Paket in separate Kopie, damit loop() sicher darauf zugreifen kann
                memcpy(bleRxReady, bleRxBuf, bleRxLen + 1);
                bleNewMsg = true;
            }
            bleRxLen = 0;
        } else if (bleRxLen < (uint8_t)MAX_PACKET_SIZE) {
            bleRxBuf[bleRxLen++] = ch;
        }
    }
}

// ==========================================================
// BLE: CLIENT-CALLBACKS  (Disconnect erkennen)
// ==========================================================
...
// ==========================================================
void doConnect() {
    Serial.println("[BLE] Verbinde zu HM-10...");
    ...
    if (!pClient->connect(NimBLEAddress(SLAVE_ADDRESS, BLE_ADDR_PUBLIC))) {
        Serial.println("[BLE] Verbindung fehlgeschlagen, naechster Versuch...");
        return;
    }
    ...
    if (!pChar) {
        Serial.println("[BLE] Characteristic nicht gefunden");
        pClient->disconnect();
        return;
    }
...
void handleInput(const char *input) {
    ...
    if (bleConnected && pChar) {
        pChar->writeValue(packet, false);
        Serial.print("[SEND] ");
        Serial.println(packet);
    } else {
        Serial.println("[WARN] Nicht verbunden – Nachricht nicht gesendet.");
    }
}
...
// Zeigt "nicht verbunden" Status oben rechts wenn BLE weg ist
void drawBleStatus() {
    if (!bleConnected) {
        u8g2.drawStr(90, 10, "NO BLE");
    }
}
...
void setup() {
    ...
    // SPI für MFRC522 (ESP32 VSPI: SCK=18 MISO=19 MOSI=23)
    ...
    NimBLEDevice::init("ESP32_ALUHUT");
    doConnect();

    Serial.println("ESP32 MASTER gestartet.");
...
void loop() {
    ...
    // --- BLE EINGEHENDE NACHRICHT VERARBEITEN ---
    if (bleNewMsg) {
        bleNewMsg = false;
        char decoded[MAX_MSG_SIZE + 1];
        if (parseIncoming(bleRxReady, decoded)) {
    ...
    if (currentMode == MENU) {
        if (now - lastNavTime > navMs) {
            if      (yVal < JOY_LOW  && menuSelection > 0) { menuSelection--; lastNavTime = now; }
            else if (yVal > JOY_HIGH && menuSelection < 2) { menuSelection++; lastNavTime = now; }
        }
        if (pressed) {
    ...
    // --- KEYBOARD ---
        ...
        if (now - lastNavTime > navMs) {
            if      (xVal < JOY_LOW  && cursorX > 0) { cursorX--; lastNavTime = now; }
            else if (xVal > JOY_HIGH && cursorX < 9) { cursorX++; lastNavTime = now; }
            else if (yVal < JOY_LOW  && cursorY > 0) { cursorY--; lastNavTime = now; }
            else if (yVal > JOY_HIGH && cursorY < 2) { cursorY++; lastNavTime = now; }
        }
        if (pressed) {
            ...
            if (sel == '<') {
                ...
                if (inputLen > 0) {
                    handleInput(inputBuffer);        // senden (BLE)
                    saveMessageToInbox(inputBuffer); // auch lokal in Inbox
                    drawSuccessScreen();
                    delay(900);
                    clearInput();
                    currentMode = MENU;
    ...
    // --- INBOX ---
        ...
        if (messageCount > 0 && now - lastNavTime > navMs) {
            if      (yVal < JOY_LOW  && inboxSelection > 0)                { inboxSelection--; lastNavTime = now; }
            else if (yVal > JOY_HIGH && inboxSelection < messageCount - 1) { inboxSelection++; lastNavTime = now; }
